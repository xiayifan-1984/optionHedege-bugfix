...
int			Transform::Instance(tagTradeUserInfo* pInfo, directStrategy* pSpi,CTwapUnit* pTwapUnit, const char* pszbroker)
{
	m_oUserInfo = *pInfo;
	m_pTradeSpi = pSpi;
	m_pTwapUnit = pTwapUnit;
	...
	_snprintf(szConsumeName, 254, "twap_%s_%d_%d",pInfo->User, oDt.dateToInt(), oDt.timeToInt() );
...
int	Transform::onConsumeCB(const char* topic, int partition, long long offset, int buflen, const char* pbuf)
{
	...
	case 40:
	{
		...
		tagXTTwapQryOrderField* pField = (tagXTTwapQryOrderField*)rr.ReadSolidPtr();
		int reqid = rr.ReadInt32();
		if (!rr.IsError())
		{
			m_pTwapUnit->DoReqQryTwapOrder(pField, reqid);
		}
	}
	break;
	case 41:
	{
		...
		rr.BindStream(pcontent, contentlen);
		int argc = rr.ReadInt32();
		tagXTTwapInputOrderField* pField = (tagXTTwapInputOrderField*)rr.ReadSolidPtr();
		int reqid = rr.ReadInt32();
		if (!rr.IsError())
		{
			m_pTwapUnit->DoReqTwapOrderInsert(pField, reqid);
		}
	}
	break;
	case 42:
	{
		...
		rr.BindStream(pcontent, contentlen);
		int argc = rr.ReadInt32();
		tagXTTwapOrderActionField* pField = (tagXTTwapOrderActionField*)rr.ReadSolidPtr();
		int reqid = rr.ReadInt32();
		if (!rr.IsError())
		{
			m_pTwapUnit->DoReqTwapOrderAction(pField, reqid);
		}
	}
	break;
	case 43:
	{
		...
		if (!rr.IsError())
		{
			m_pTwapUnit->DoReqQryTwapOrderDetail(pField, reqid);
		}
	}
	break;
	default:
...
void		Transform::OnRspOrderInsert(CThostFtdcInputOrderField *pInputOrder, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	...
	if (pInputOrder)
	{
		...
		if (m_pTwapUnit)
		{
			m_pTwapUnit->handleOrderRtn(oField.ExCode, oField.OrderRef);
		}
...
void 		Transform::OnRtnTwapOrder(tagXTTwapOrderField* pOrder)
{
	char tempbuf[4096] = { 0 };
	RpcWrite rw(tempbuf, 4095);
	rw.WriteInt32(1);
	...
	if (!rw.IsError())
	{
		sendResponse("431", 2, 45, tempbuf, rw.GetOffset());
	}
}

void 		Transform::OnRspQryTwapOrder(tagXTTwapOrderField* pOrder, tagXTRspInfoField* pRspInfo, int nRequestID, bool bIsLast)
{
	char tempbuf[4096] = { 0 };
	RpcWrite rw(tempbuf, 4095);
	rw.WriteInt32(4);
	if (pOrder)
	{
		rw.WriteSolidPtr((char*)pOrder, sizeof(tagXTTwapOrderField));
	}
	else
	{
		rw.WriteSolidPtr(0, 0);
	...
	if (pRspInfo && pRspInfo->ErrorID != 0)
	{
		rw.WriteSolidPtr((char*)pRspInfo, sizeof(tagXTRspInfoField));
	}
	else
	{
		rw.WriteSolidPtr(0, 0);
	...
	if (!rw.IsError())
	{
		sendResponse("431",2, 40, tempbuf, rw.GetOffset());
	}
}

void 		Transform::OnRspQryTwapOrderDetail(tagXTTwapOrderInsertRtnField* pOrder, tagXTRspInfoField* pRspInfo, int nRequestID, bool bIsLast)
{
	char tempbuf[4096] = { 0 };
	RpcWrite rw(tempbuf, 4095);
	rw.WriteInt32(4);
	if (pOrder)
	{
		rw.WriteSolidPtr((char*)pOrder, sizeof(tagXTTwapOrderInsertRtnField));
	}
	else
	{
		rw.WriteSolidPtr(0, 0);
	...
	if (!rw.IsError())
	{
		sendResponse("431",2, 43, tempbuf, rw.GetOffset());
	}
}

void 		Transform::OnRtnTwapOrderInsert(tagXTTwapOrderInsertRtnField* pOrder)
{
	char tempbuf[4096] = { 0 };
	RpcWrite rw(tempbuf, 4095);
	rw.WriteInt32(1);
	...
	if (!rw.IsError())
	{
		sendResponse("431",2, 46, tempbuf, rw.GetOffset());
	}
}
