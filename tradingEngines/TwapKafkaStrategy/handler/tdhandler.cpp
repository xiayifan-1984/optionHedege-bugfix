...
int			tdhandler::inner_Instance()
{
	...
	int iret = m_pTransform->Instance(&oUserInfo, m_pStrategy, m_pTwapUnit, szbroker);
	if (iret <0)
	{
		return -1;
	}
	...
	//[5]
	iret = m_pTwapUnit->Instance(&oUserInfo, m_pStrategy, m_pTransform, m_pTimeSection);
	if (iret <0)
	{
		return -2;
	}
...
void 	tdhandler::handleTwapOrder()
{
	if (m_pTwapUnit)
	{
		m_pTwapUnit->OnPeriodCall();
	}
