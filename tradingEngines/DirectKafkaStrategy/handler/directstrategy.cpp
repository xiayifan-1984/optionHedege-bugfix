...
{
	logger = optionHedge::memoryManage::Log::getLogger("directstrategy");
	m_pTransform = 0;
	m_mapReqID.clear();
	m_tickers.clear();
}
...
void		directStrategy::init()
{
	...
	//m_tickers.push_back("cu1909");
	//m_tickers.push_back("sn1909");
	//m_tickers.push_back("fu1909");

	util->subscribeMarketData(m_tickers, SOURCE_INDEX);
...
{
	for (auto it = m_tickers.begin(); it != m_tickers.end(); ++it)
	{
		if (strcmp(pcode, (*it).c_str()))
		{
			return true;
		}
	}
